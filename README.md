...
- **[Alma](https://www.alma.careers)** ([code](https://github.com/Dremix10/WarmIntro)) — AI recruiting-outreach agent for investment banking. Finds verified bankers, drafts and fact-checks cold emails, tracks the whole funnel.
  - A team of specialized Claude agents (Researcher, Correspondent, Critic, Watcher, Curator) behind one deterministic planner.
  - Two real self-learning loops, not just an LLM wrapper: one closes fully automatically, recalibrating targeting weights every week from actual reply/referral outcomes; the other is a self-audit checking whether its own reviewer's score actually predicts real replies.
- **[Tichu](https://aegist.dev)** ([code](https://github.com/Dremix10/cyprus)) — real-time multiplayer card game with an AI opponent (Information-Set Monte Carlo Tree Search). Node/Socket.IO backend, React frontend, self-hosted.
- **[DOBI](https://www.bookwithdobi.com/chat/cv-sample)** ([code](https://github.com/Dremix10/agent-jones)) — AI booking assistant: qualifies leads and books appointments through a conversation state machine, with structured output and a fallback parser for bad model responses. Grew out of the linked hackathon prototype; DOBI itself is closed-source, running for paying clients.
